...
class TestCreatePersonalEvent:
    ...
    def test_create_personal_event_positive(self):
        ...
        with allure.step("Отправка POST запроса для создания события"):
            response = requests.post(
                API_URL_CREATE, headers=API_HEADERS, json=self.BODY
            )
    ...
    @allure.issue(
        "BUG-1",
        "Система позволяет создавать события "
        "с отсутствующими обязательными полями",
    )
    
    @pytest.mark.xfail(
        reason="Известный баг BUG-1: отсутствует валидация обязательных полей"
    )
    @allure.description(
        """
    ...
    def test_create_personal_event_negative(self):
        ...
        with allure.step("Отправка POST запроса с невалидными данными"):
            response = requests.post(
                API_URL_CREATE, headers=API_HEADERS, json=invalid_body
            )
            allure.attach(
                json.dumps(invalid_body, indent=2), name="Invalid Request Body"
            )
            allure.attach(
                f"Response status: {response.status_code}",
                name="Response Status"
        ...
        with allure.step("Проверка кода ошибки"):
            assert response.status_code in [
                400,
                422,
            ], f"Ожидалась ошибка 400 или 422, " \
    ...
    def test_create_personal_event_without_authentication(self):
        ...
        with allure.step("Отправка запроса без аутентификации"):
            response = requests.post(
                API_URL_CREATE, headers=headers_without_auth, json=self.BODY
            )
            allure.attach(
                json.dumps(headers_without_auth, indent=2),
                name="Headers without Auth"
            )
            allure.attach(
                f"Response status: {response.status_code}",
                name="Response Status"
        ...
        with allure.step("Проверка ошибки аутентификации"):
            assert response.status_code in [
                401,
                403,
            ], f"Ожидалась ошибка аутентификации 401 или 403, " \
                f"но получен {response.status_code}"
    ...
    def test_create_personal_event_response_time(self):
        ...
        with allure.step("Проверка времени ответа"):
            ...
            ), f"Запрос не выполнен успешно: {response.status_code}"
...
class TestRemovePersonalEvent:
    ...
    def test_remove_personal_event_success(self):
        ...
        body = {"id": self.EVENT_ID, "startAt": "2025-09-28T19:00:00+03:00"}
        ...
        # Проверка статус кода
        with allure.step("Проверка статус кода"):  # ← Убрали f
            assert response.status_code == 200, (
                f"Ожидался статус 200, но получен {response.status_code}. "
                f"Response: {response.text}"
            )
